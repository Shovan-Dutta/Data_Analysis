...
def child_main():
    ...
    data2 = data.drop('class', axis=1).astype(float) #keeping a separate copy of only the features
    print("The 'child-all.csv' is loaded.")
    choice = 0
    std1 = [] #to store the standard deviation after performing normalization
    while(choice != 100):
        print("\n\n****************************** Main Menu ******************************")
        ...
        if choice == 1:
            ...
            if len(std1) != 0:
                ...
                for c in most_imp:
                    file[c]=list(data2[c])
                file["class"] = list(data["class"])
                file.to_csv("child_subset.csv",index=False)
                dataplot = sb.heatmap(file.corr(), annot=True)
                mp.show()
            else:
                print("Without normalizing the dataset this operation is not possible")
...
def leukemia_main():
    ...
    data2 = data.drop('class', axis=1) #keeping a separate copy of only the features
    print("The 'leukemia.csv' is loaded.")
    choice = 0
    std1 = [] #to store the standard deviation after performing normalization
    while(choice != 100):
        print("\n\n****************************** Main Menu ******************************")
        ...
        if choice == 1:
            ...
            if len(std1) != 0:
                ...
                for c in most_imp:
                    file[c]=list(data2[c])
                file["class"] = list(data["class"])
                file.to_csv("leukemia_subset.csv",index=False)
                dataplot = sb.heatmap(file.corr(), annot=True)
                mp.show()
            else:
                print("Without normalizing the dataset this operation is not possible")
...
def mll_leukemia_main():
    data= pd.read_csv("mll-leukemia.csv")
    data = data.replace('?',np.nan) # replaces the 'None' string to null
    data2 = data.drop('class', axis=1).astype(float) #keeping a separate copy of only the features
    print("The 'mll_leukemia.csv' is loaded.")
    choice = 0
    std1 = [] #to store the standard deviation after performing normalization
    while(choice != 100):
        print("\n\n****************************** Main Menu ******************************")
        ...
        if choice == 1:
            ...
            if len(std1) != 0:
                ...
                for c in most_imp:
                    file[c]=list(data2[c])
                file["class"] = list(data["class"])
                file.to_csv("mll_leukemia_subset.csv",index=False)
                dataplot = sb.heatmap(file.corr(), annot=True)
                mp.show()
            else:
                print("Without normalizing the dataset this operation is not possible")
...
def main_caller(file):
    if file == "dlbcl":
        ...
        child_main()
    elif file == "mll_leukemia":
        mll_leukemia_main()

parser = argparse.ArgumentParser(description="Toolkit for cancer datasets")
parser.add_argument("--data",type = str, default = "dlbcl",choice = ["dlbcl","leukemia","child","mll_leukemia"],help  = "Choose the dataset")
args = parser.parse_args()
main_caller(args.data)